...
async function main() {
  ...
  console.log("Deploying contracts with auditor: ", auditor.address);
  ...
  await hre.ethernal.push({
    name: "Projects",
    address: projects.target as string,
  });
  ...
  await hre.ethernal.push({
    name: "ProjectToken",
    address: projectToken.target as string,
  });

  const CarbonCreditTokenFactory = await ethers.getContractFactory(
    "CarbonCreditToken"
  );
  const cctToken = await CarbonCreditTokenFactory.deploy(
    auditor.address,
    projectToken.target
  );
  ...
  await hre.ethernal.push({
    ...
    address: cctToken.target as string,
  });
  ...
  await hre.ethernal.push({
    name: "Marketplace",
    address: marketplace.target as string,
  });

  const LiquidityProviderTokenFactory = await ethers.getContractFactory(
    "LiquidityProviderToken"
  );
  const lpToken = await LiquidityProviderTokenFactory.deploy(
  ...
  await hre.ethernal.push({
    ...
    address: lpToken.target as string,
  });
